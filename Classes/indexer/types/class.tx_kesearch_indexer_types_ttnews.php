...
class tx_kesearch_indexer_types_ttnews extends tx_kesearch_indexer_types
{
     ...
     * @return array
     */
    public function getParamsForHrDateSingleView($tstamp)
    {
        ...
        if ($GLOBALS['TYPO3_DB']->sql_num_rows($res)) {
            while (($newsRecord = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res))) {
                ...
                if ($this->indexerConfig['index_news_category_mode'] == '2') {
                    $enableFields = BackendUtility::BEenableFields('tt_news_cat')
                        . BackendUtility::deleteClause('tt_news_cat');
                    ...
                    $resCat = $GLOBALS['TYPO3_DB']->exec_SELECT_mm_query(
                        ...
                        'tt_news_cat_mm',
                        'tt_news_cat',
                        ' AND tt_news.uid = ' . $newsRecord['uid'] . $enableFields,
                        '',
                        '',
                        ''
                    );
                    ...
                    if ($GLOBALS['TYPO3_DB']->sql_num_rows($resCat)) {
                        ...
                        while ($newsCat = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($resCat)) {
                            ...
                            if (TYPO3\CMS\Core\Utility\GeneralUtility::inList(
                                $this->indexerConfig['index_news_category_selection'],
                                $newsCat['uid']
                            )
                            ) {
                                $isInList = true;
     ...
     * @since 04.12.13
     */
    public function getSingleViewPageFromCategories($uid, $getAll = false)
    {
    ...
    {
        $addWhere = ' AND tt_news_cat.deleted=0' . BackendUtility::BEenableFields('tt_news_cat');
        $res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
            'uid,parent_category,single_pid',
            'tt_news_cat',
            'tt_news_cat.uid=' . $currentCategory . $addWhere
        );
