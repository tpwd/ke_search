...
class IndexRepository
{
    ...
    public function findOneByHashAndModificationTime(string $hash, $mtime)
    {
        ...
        return $queryBuilder
            ...
            ->where(
                ...
                $queryBuilder->expr()->eq(
                    'sortdate',
                    $queryBuilder->createNamedParameter($mtime, Connection::PARAM_INT)
                )
            )
            ->setMaxResults(1)
            ->executeQuery()
    ...
    public function deleteByUniqueProperties(int $origUid, int $pid, string $type, int $language)
    {
        ...
        return $queryBuilder
            ->delete($this->tableName)
            ->where(
                $queryBuilder->expr()->eq('orig_uid', $queryBuilder->createNamedParameter($origUid, Connection::PARAM_INT)),
